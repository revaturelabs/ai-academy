...
Set `$bias_defense_applied: true` in `quiz.meta.json` only after the shuffle has run. The quiz-critic refuses any quiz where the sidecar's marker is missing or false.
...
Two files in `<topic-dir>/artifacts/`:
