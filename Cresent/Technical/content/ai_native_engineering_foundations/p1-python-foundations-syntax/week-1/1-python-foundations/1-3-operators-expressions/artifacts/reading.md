...
**Logical operators** combine or invert `bool` values into compound conditions [1][3]. There are exactly three, written as plain English words: `A and B` is `True` only when *both* are true; `A or B` is `True` when *at least one* is true; `not A` flips the value.
