...
**Indexing & slicing.** Because a tuple is a sequence, everything you learned about reading a list in 4.1 applies unchanged: indexing starts at `0`, negative indices count from the end, slicing uses `[start:stop:step]` and returns a brand-new tuple, and you can loop and test membership [2]:
...
**Nested tuples.** A tuple can contain other tuples, so you can represent tables or grids with no new syntax. Reading a nested value is **chained indexing** — the first index selects the inner tuple, the second reaches inside it — and immutability nests too [2]:
...
**Operations & methods.** Three operations round out the toolkit, all using operators you already know from lists and P1 [2]:
...
**Tuple vs list.** Ask one question — *will the collection's membership change?* [1] Decide like this:
